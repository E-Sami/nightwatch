...
module.exports.createClient = function({
  ...
  const exported = {
    ...
    /**
     * Drain pending Nightwatch commands at a Cucumber scenario boundary without ending the session.
     * @param {{timeoutMs?: number}} [options]
     */
    async drainCommandQueue(options = {}) {
      await client.queue.drainScenarioQueue(options);

      if (client.api && client.api.globals) {
        client.api.globals.__nightwatchScenarioEpoch =
          (client.api.globals.__nightwatchScenarioEpoch || 0) + 1;
      }
    }
  };
