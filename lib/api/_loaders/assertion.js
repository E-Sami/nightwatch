  ...
  createQueuedCommandFn({parent, apiToReturn}) {
    ...
    return function queuedCommandFn({negate, args}) {
      ...
      if (isES6Async) {
        ...
        if (commandQueue.tree.cucumberRunner) {
          const onCommandFinished = ({node: finishedNode}) => {
            if (finishedNode !== node) {
              return;
            }

            commandQueue.tree.removeListener('asynctree:command:finished', onCommandFinished);

            if (!node.promiseSettled) {
              node.deferred.resolve(node.resolveValue);
            }
          };

          commandQueue.tree.on('asynctree:command:finished', onCommandFinished);
        } else {
