...
class CommandQueue extends EventEmitter {
  ...
  add(command) {
    ...
    if (this.client && this.client.__acceptCommands === false) {
      deferred.resolve(CommandQueue.suppressedScenarioBoundaryResult(commandName));
      return new Node({
        name: commandName,
        ...
        rejectPromise,
        compatMode,
        suppressedAtScenarioBoundary: true
      });
    }
   ...
   * Suspends new commands, waits briefly for in-flight work, clears pending nodes and stale listeners.
   *
   * @param {{timeoutMs?: number}} [options]
   */
  async drainScenarioQueue({timeoutMs = 250} = {}) {
    if (this.client) {
      this.client.__acceptCommands = false;
    }
    ...
    for (const child of [...(this.tree.rootNode.childNodes || [])]) {
      if (child.deferred && !child.promiseSettled) {
    ...
    if (this.client) {
      this.client.__scenarioBoundaryGeneration++;
    }
  }

  resumeScenarioCommands() {
    if (this.client) {
      this.client.__acceptCommands = true;
    }
  }

  static suppressedScenarioBoundaryResult(commandName) {
