...
class AsyncTree extends EventEmitter{
  ...
  constructor({compatMode, foreignRunner, cucumberRunner, mochaRunner, client}) {
    super();
  ...
  async runChildNode(node) {
    if (node.suppressedAtScenarioBoundary) {
      node.done = true;
      node.resolve(suppressedScenarioBoundaryResult(node.name));
    ...
    if (result instanceof Error) {
      ...
      if (err.scenarioBoundaryDrain) {
        node.resolve(err);
        this.emit('asynctree:command:finished', {node, result: err});
    ...
    await this.runCommandFinishedHook(node, result);
  ...
  async runCommandFinishedHook(node, result) {
    const hook = this.client && this.client.settings && this.client.settings.globals
      ? this.client.settings.globals.onCommandFinished
      : null;
    ...
    const payload = AsyncTree.createCommandFinishedPayload(node, result);

    try {
      await hook.call(this.client.settings.globals, this.client.api, payload);
    } catch (err) {
      const error = new Error(`Error during onCommandFinished() global hook: ${err.message}`);
      Logger.error(error);
    }
  }
  ...
  static sanitizeArguments(node) {
    ...
    return args.map(arg => {
      ...
      if (node.redact && typeof arg === 'string') {
        return '**********';
      }
  ...
  static createCommandFinishedPayload(node, result) {
    ...
    return {
      ...
      args: AsyncTree.sanitizeArguments(node),
...
function suppressedScenarioBoundaryResult(commandName) {
  if (commandName === 'pause') {
    return null;
  }

  if (commandName === 'isEnabled' || commandName === 'isPresent' || commandName === 'isVisible') {
    return false;
  }
