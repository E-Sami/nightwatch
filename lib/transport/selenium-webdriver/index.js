...
class Transport extends BaseTransport {
  ...
  async createSession({argv, moduleKey, reuseBrowser = false}) {
    ...
    try {
      ...
      const reuseCheckTime = new Date() - t2;
      const tDriverStart = new Date();
      ...
      if (wasReused) {
        ...
        if (this.settings.webdriver.debug_timing) {
          // eslint-disable-next-line no-console
          console.info(`[Timing] Driver reused (check time: ${reuseCheckTime}ms)`);
        }
      } else {
        // Need to create new driver.
        this.driver = await this.getDriver({options, reuseBrowser});
      }
      timing.driverCreation = new Date() - tDriverStart;
      ...
      timing.sessionExport = new Date() - t3;
      ...
      if (this.settings.webdriver.debug_timing) {
        // eslint-disable-next-line no-console
        console.info(`[Timing] driverCreation: ${timing.driverCreation}ms ${wasReused ? '(reused)' : '(new session)'}`);
        // eslint-disable-next-line no-console
        console.info(`[Timing] sessionExport (capabilities only): ${timing.sessionExport}ms`);
      }
