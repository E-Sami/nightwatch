...
class Transport extends BaseTransport {
  ...
  async createSession({argv, moduleKey, reuseBrowser = false}) {
    ...
    const logTiming = (stage, start) => {
      ...
      if (this.settings.webdriver.debug_timing) {
        Logger.info(`[Timing] ${stage}: ${elapsed}ms`);
      }
    };
    ...
    try {
      ...
      if (this.settings.webdriver.debug_timing) {
        Logger.info(`[Timing] Total connection time: ${timing.total}ms`);
        Logger.info(`[Timing] Detailed breakdown: options=${timing.createOptions}ms, service=${serviceTime}ms, driver=${timing.driverCreation}ms, export=${timing.sessionExport}ms`);
      } else {
        // Show brief timing info in connection message
        Logger.info(`[Timing] Connection breakdown ${timingBreakdown}`);
      }
  ...
  async showConnectInfo({startTime, port, host, start_process, sessionInfo, timingBreakdown}) {
    if (!this.parallelMode) {
      ...
      this.showConnectSpinner(`Connected to ${colors.stack_trace(start_process ? this.serviceName : host)} on port ${colors.stack_trace(port)} ${colors.stack_trace('(' + totalTime + 'ms)')}.${timingInfo}`);
    }
