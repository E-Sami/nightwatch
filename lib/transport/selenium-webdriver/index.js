...
class Transport extends BaseTransport {
  ...
  async createSession({argv, moduleKey, reuseBrowser = false}) {
    ...
    try {
      ...
      // Note: WebDriver.createSession() may return quickly, but actual HTTP request
      // happens when we first interact with the driver (in session.exported())
      ...
      // Calculate actual driver creation time (includes HTTP POST to /session)
      // This is the time from starting driver creation until we have session info
      const actualDriverTime = new Date() - t2;
      timing.driverCreation = actualDriverTime;
      
      // Session export time is just the time to get capabilities after session exists
      const exportTime = new Date() - t3;
      timing.sessionExport = exportTime;
      
      if (this.settings.webdriver.debug_timing) {
        // eslint-disable-next-line no-console
        console.info(`[Timing] driverCreation (actual): ${actualDriverTime}ms`);
        // eslint-disable-next-line no-console
