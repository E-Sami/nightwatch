...
After(async function(testCase) {
  ...
  if (this.client && typeof this.client.drainCommandQueue === 'function') {
    const timeoutMs = result && result.status === 'PASSED' ? 250 : 2000;

    try {
      await this.client.drainCommandQueue({timeoutMs});
    } catch (err) {
      // Draining must not fail the scenario After hook; stale commands are discarded.
    }
  }
